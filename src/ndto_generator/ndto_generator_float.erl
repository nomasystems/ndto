...
is_valid(Prefix, #{type := float} = Schema) ->
    ...
    TrueClause =
        erl_syntax:clause(
            ...
            ndto_generator:type_guard(float),
            ndto_generator:chain_conditions(FunName, ValidationConditions, 'andalso')
        ),
    FalseClause = ndto_generator:false_clause(<<FunName/binary, ".type">>, "Value is not a float"),
...
is_valid_(_Prefix, multiple_of, _MultipleOf, _Schema) ->
    undefined;
